...
const BudgetSupplementalPage = () => {
  ...
  const handleBSPAction = async (info, action) => {
    ...
    try {
      ...
      const errMsg = error.response?.data?.error || error.response?.data?.message || `Error ${actionPresent} Budget Supplemental`;
      toast.error(errMsg);
    } finally {
      setIsLoading(false);
    }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-600 mt-0.5">
                  Request for additional money for certain appropriated budgets.
                </p>
              </div>
            </div>
          </div>
